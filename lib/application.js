...
Application.prototype.setupPromiseness = function () {
  ...
  self.transferPromiseness = function (target, promise) {
    self.client.transferPromiseness(target, promise)
    self.addPropertiesToPromise(target, promise)
  }
}
...
Application.prototype.start = function () {
  ...
  return self.exists()
    ...
    .then(function () { return api.initialize(self) })
...
Application.prototype.addCommands = function () {
  ...
  this.client.addCommand('getCwd', function () {
    return this.execute(function () {
      return require('electron').remote.getGlobal('process').cwd()
    }).then(getResponseValue)
  })
...
Application.prototype.addPropertiesToPromise = function (target, promise) {
  target.electron = {}
  var self = this
  Object.keys(self.electron).forEach(function (module) {
    target.electron[module] = self.electron[module]
  })
}
