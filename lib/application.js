  ...
  return new Promise(function (resolve, reject) {
    ...
    if (process.platform === 'win32') {
      ...
      args.forEach(function(arg) {
        ...
        if (splitted[0] === 'spectron-path') {
          launcherPath = splitted[1];
        } else if (splitted[0].indexOf('spectron-env') !== -1) {
          process.env[name.substring(15)] = splitted[1];
        } else if (splitted[0] === 'spectron-arg0') {
          darwinArgs.push('app=' + splitted[1]);
        } else if (splitted[0].indexOf('spectron-arg') !== -1) {
          darwinArgs.push(splitted[1]);
        }
      });
